...
const MainContent: React.FC = () => {
	...
	useEffect(() => {
		if (!isValidBrowserAgent() && isAndroidDevice()) {
		...
		return (
			<div className='flex flex-col items-center justify-center h-screen bg-background-light dark:bg-background-dark p-8 text-center'>
				...
				<p className='text-[11px] text-gray-400 leading-relaxed max-w-[280px]'>
					...
					오류가 발생하면 우측 상단 <b>'...'</b> 버튼을 눌러 <b>'다른 브라우저로 열기'</b>를 선택하거나 위의 버튼을
					이용해 주세요.
				</p>
			</div>
		);
		...
		<div className='relative flex h-auto min-h-screen w-full flex-col overflow-x-hidden mx-auto max-w-md bg-background-light dark:bg-background-dark pb-24 shadow-2xl transition-colors duration-200'>
			...
			<main>
				<div className='px-4 pt-6 pb-2'>
					...
					<p className='text-gray-600 dark:text-gray-400 text-base font-normal leading-normal'>{t.scheduleVisit}</p>
				</div>
...
function isValidBrowserAgent() {
	const userAgent = navigator.userAgent.toLowerCase();
	if (userAgent.indexOf('kakaotalk') != -1) return false;
	if ((userAgent.indexOf('opera') || userAgent.indexOf('opr')) != -1) return true;
	...
	if (userAgent.indexOf('msie') != -1 || !!(document as any).documentMode == true) return true;
	return false;
}
